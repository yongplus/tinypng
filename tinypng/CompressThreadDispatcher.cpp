...
void CompressThreadDispatcher::run() {
	...
	elapsedTimer->start();
	qDebug() << "total row:" << total << runThreadNum << qMin(runThreadNum, total);

	runThreadNum = qMin(runThreadNum, total);
...
void CompressThreadDispatcher::doneTask(QVariant variant) {
	...
	if (result.errcode == 0) {
		...
		QString text = QString("%1/%2压缩成功:%3(%4),%5%").arg(finishNum).arg(rowCount).arg(row.path).arg(size).arg(rate);
		this->console->info(text);
		this->model->setData(this->model->index(result.row, 3), QVariant(brush), 9);
	}
	else {
		failedNum += 1;
		QString text = QString("%1/%2压缩失败:%3,%4").arg(finishNum).arg(rowCount).arg(row.path).arg(result.errmsg);
